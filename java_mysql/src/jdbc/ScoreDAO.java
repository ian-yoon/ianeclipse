...
public class ScoreDAO {
	...
	public Vector search_score(String name) {
		...
		try {
			...
			String sql = "select student_no, name, kor, eng, mat,"
					+ " (kor+eng+mat) tot, round(((kor+eng+mat)/3.0),1) avg from scores where name like ? order by name";
	...
	public Vector list_score() {
		...
		try {
			conn = DB.dbConn();
			String sql = "select student_no, name, kor, eng, mat,"
					+ " (kor+eng+mat) tot, round(((kor+eng+mat)/3.0),1) avg from scores order by name";
			System.out.println(sql);
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while (rs.next()) {
